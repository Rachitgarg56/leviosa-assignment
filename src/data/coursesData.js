const coursesData = [
    {
      name: "Web Development Bootcamp",
      description: "Learn the fundamentals of web development using HTML, CSS, and JavaScript.",
      ytv: "https://www.youtube.com/embed/G10vMyRj8Aw",
      creatorName: "Sarah Johnson",
      creatorProfession: "Full Stack Developer",
      creatorProfile: "https://pbs.twimg.com/profile_images/1271544502340198400/8Fq7zjbq_400x400.jpg",
      stars: 5,
      usersReviewed: 1500,
      level: "All levels",
      status: false,
      language: "English",
      img: "https://images.unsplash.com/photo-1457305237443-44c3d5a30b89?q=80&w=2074&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      content: [
        { name: "HTML Basics", description: "Introduction to HTML", ytv: "https://www.youtube.com/embed/z9FeTl5eq9E", status: false, color: "orange" },
        { name: "CSS Basics", description: "Learn CSS for styling", ytv: "https://www.youtube.com/embed/ZxKM3DCV2kE", status: false, color: "yellow" },
        { name: "JavaScript Basics", description: "Introduction to JavaScript", ytv: "https://www.youtube.com/embed/dX8396ZmSPk", status: false, color: "red" },
        { name: "Responsive Design", description: "Learn responsive web design", ytv: "https://www.youtube.com/embed/Q33KBiDriJY", status: false, color: "pink" },
      ],
    },
    {
      name: "Python for Data Science",
      description: "Master Python programming for data analysis and visualization.",
      ytv: "https://www.youtube.com/embed/i9uAgkKEuNU",
      creatorName: "Michael Smith",
      creatorProfession: "Data Scientist",
      creatorProfile: "https://cdn.shopify.com/s/files/1/0313/7676/9157/files/Player-Profiles_Michael-Smith_1000x1000px.jpg?v=1687917909%22",
      stars: 4,
      usersReviewed: 1200,
      level: "Intermediate",
      status: false,
      language: "English",
      img: "https://images.unsplash.com/photo-1649180556628-9ba704115795?q=80&w=2062&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      content: [
        { name: "Python Basics", description: "Learn Python fundamentals", ytv: "https://www.youtube.com/embed/ZcaKgqXsEbA", status: false, color: "orange" },
        { name: "Data Analysis", description: "Pandas for data analysis", ytv: "https://www.youtube.com/embed/ua-CiDNNj30", status: false, color: "yellow" },
        { name: "Data Visualization", description: "Plotting with Matplotlib", ytv: "https://www.youtube.com/embed/-ETQ97mXXF0", status: false, color: "red" },
      ],
    },
    {
      name: "Full Stack Development",
      description: "Learn to build scalable full-stack web applications.",
      ytv: "https://www.youtube.com/embed/ZxKM3DCV2kE",
      creatorName: "Emily Davis",
      creatorProfession: "Software Engineer",
      creatorProfile: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYDyfV5EumlcN5ozihDQn5qh9LFyz-9zfadg&s",
      stars: 3,
      usersReviewed: 2000,
      level: "Beginner",
      status: false,
      language: "English",
      img: "https://plus.unsplash.com/premium_photo-1685086785636-2a1a0e5b591f?q=80&w=1932&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      content: [
        { name: "Frontend Basics", description: "HTML, CSS, JS overview", ytv: "https://www.youtube.com/embed/0GYU5JOYkdI", status: false, color: "orange" },
        { name: "Backend Basics", description: "Introduction to Node.js", ytv: "https://www.youtube.com/embed?list=PLEiEAq2VkUULCC3eEATL4zzuapTjmo1Z_", status: false, color: "yellow" },
        { name: "Databases", description: "Database concepts and MongoDB", ytv: "https://www.youtube.com/embed/8sXRyHI3bLw", status: false, color: "red" },
        { name: "APIs", description: "Building REST APIs", ytv: "https://www.youtube.com/embed/u1TLyC04ktE", status: false, color: "pink" },
      ],
    },
    {
      name: "UI/UX Design Essentials",
      description: "Learn the basics of UI/UX design to create stunning interfaces.",
      ytv: "https://www.youtube.com/embed/dX8396ZmSPk",
      creatorName: "David Brown",
      creatorProfession: "UI/UX Designer",
      creatorProfile: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSxi01fApN6wDu1hzVBXLJ7UICS1bWfUfYg3A&s",
      stars: 4,
      usersReviewed: 800,
      level: "Intermediate",
      status: false,
      language: "English",
      img: "https://images.unsplash.com/photo-1581291518857-4e27b48ff24e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      content: [
        { name: "UX Basics", description: "Understanding user experience", ytv: "https://www.youtube.com/embed/3u9jXGwoWC8", status: false, color: "orange" },
        { name: "Wireframing", description: "Creating wireframes", ytv: "https://www.youtube.com/embed/dYtdv--qd_U", status: false, color: "yellow" },
        { name: "UI Basics", description: "Designing user interfaces", ytv: "https://www.youtube.com/embed/t0OIk9FuMgM", status: false, color: "red" },
      ],
    },
    {
      name: "Machine Learning Mastery",
      description: "Build machine learning models from scratch.",
      ytv: "https://www.youtube.com/embed/ZKlUDaV8bh4",
      creatorName: "Jessica Lee",
      creatorProfession: "Machine Learning Engineer",
      creatorProfile: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQRlXf4VIXakCEHWx-kh82oklSk1oVIelRu-A&s",
      stars: 5,
      usersReviewed: 1800,
      level: "All levels",
      status: false,
      language: "English",
      img: "https://plus.unsplash.com/premium_photo-1682124651258-410b25fa9dc0?q=80&w=1921&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      content: [
        { name: "ML Basics", description: "Introduction to machine learning", ytv: "https://www.youtube.com/embed/j87TPH5b6Ds", status: false, color: "orange" },
        { name: "Linear Regression", description: "Understanding regression models", ytv: "https://www.youtube.com/embed?list=PLEiEAq2VkUULYYgj13YHUWmRePqiu8Ddy", status: false, color: "yellow" },
        { name: "Classification", description: "Building classifiers", ytv: "https://www.youtube.com/embed/bmmQA8A-yUA", status: false, color: "red" },
        { name: "Deep Learning", description: "Introduction to neural networks", ytv: "https://www.youtube.com/embed/PkAx2YZIWwQ", status: false, color: "pink" },
      ],
    },
    {
      name: "Desarrollo Web con React",
      description: "Aprende a construir aplicaciones web interactivas usando React.",
      ytv: "https://www.youtube.com/embed/7C2b7bK4YfE",
      creatorName: "Lucía Martínez",
      creatorProfession: "Desarrolladora Frontend",
      creatorProfile: "https://example.com/profile_lucia.jpg",
      stars: 4,
      usersReviewed: 600,
      level: "Intermediate",
      status: false,
      language: "Spanish",
      img: "https://images.unsplash.com/photo-1633356122102-3fe601e05bd2?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8cmVhY3Rqc3xlbnwwfHwwfHx8MA%3D%3D",
      content: [
        { name: "Introducción a React", description: "Fundamentos de React", ytv: "https://www.youtube.com/embed/0sB1H4vPj1I", status: false, color: "orange" },
        { name: "Componentes en React", description: "Creación de componentes", ytv: "https://www.youtube.com/embed/ZxKM3DCV2kE", status: false, color: "yellow" },
        { name: "Estado y Props", description: "Manejo del estado en componentes", ytv: "https://www.youtube.com/embed/dX8396ZmSPk", status: false, color: "red" },
      ],
    },
    {
      name: "Ciencia de Datos con Python",
      description: "Domina el análisis de datos utilizando Python.",
      ytv: "https://www.youtube.com/embed/i9uAgkKEuNU",
      creatorName: "Carlos Fernández",
      creatorProfession: "Científico de Datos",
      creatorProfile: "https://example.com/profile_carlos.jpg",
      stars: 5,
      usersReviewed: 800,
      level: "Beginner",
      status: false,
      language: "Spanish",
      img: "https://media.istockphoto.com/id/1362729835/photo/programming-language-python-with-a-5-star-rating-and-code-elemmets-badges-3d.webp?a=1&b=1&s=612x612&w=0&k=20&c=CdeYKTMmhu_7FyDPYG0tYcmIzs3v8gOAj3zWN4isrMU=",
      content: [
        { name: "Introducción a Python", description: "Fundamentos de Python", ytv: "https://www.youtube.com/embed/ZcaKgqXsEbA", status: false, color: "orange" },
        { name: "Análisis de Datos", description: "Uso de Pandas para análisis", ytv: "https://www.youtube.com/embed/ua-CiDNNj30", status: false, color: "yellow" },
        { name: "Visualización de Datos", description: "Gráficos con Matplotlib", ytv: "https://www.youtube.com/embed/-ETQ97mXXF0", status: false, color: "red" },
      ],
    },
    {
      name: "前端开发基础",
      description: "学习HTML、CSS和JavaScript的基础知识。",
      ytv: "https://www.youtube.com/embed/G10vMyRj8Aw",
      creatorName: "王伟",
      creatorProfession: "前端工程师",
      creatorProfile: "https://example.com/profile_wang.jpg",
      stars: 4,
      usersReviewed: 500,
      level: "Beginner",
      status: false,
      language: "Chinese",
      img: "https://media.istockphoto.com/id/1284271878/photo/javascript-inscription-against-laptop-and-code-background-learn-javascript-programming.webp?a=1&b=1&s=612x612&w=0&k=20&c=IvOrAfTuemU8rfbydZyzhBHvTNEggDywPB2yjSIQ3xU=",
      content: [
        { name: "HTML基础", description: "HTML简介", ytv:"https://www.youtube.com/embed/z9FeTl5eq9E", status:false, color:"orange" },
        { name:"CSS基础", description:"学习CSS样式", ytv:"https://www.youtube.com/embed/ZxKM3DCV2kE", status:false, color:"yellow" },
        { name:"JavaScript基础", description:"JavaScript简介", ytv:"https://www.youtube.com/embed/dX8396ZmSPk", status:false, color:"red" },
       ],
    },
    {
      name:"数据科学入门",
      description:"掌握数据分析和可视化的Python编程。",
      ytv:"https://www.youtube.com/embed/i9uAgkKEuNU",
      creatorName:"李娜",
      creatorProfession:"数据科学家",
      creatorProfile:"https://example.com/profile_li.jpg",
      stars :5,
      usersReviewed :900,
      level :"Intermediate",
      status :false,
      language :"Chinese",
      img :"https://media.istockphoto.com/id/1303169188/photo/devops-concept.webp?a=1&b=1&s=612x612&w=0&k=20&c=85UduCqssTGzMREN1WK6eHP8blWQUs1uEn0X5MKeJt8=",
      content:[
         {name :"Python基础",description :"学习Python基础知识",ytv :"https://www.youtube.com/embed/ZcaKgqXsEbA",status :false,color :"orange"},
         {name :"数据分析",description :"使用Pandas进行数据分析",ytv :"https://www.youtube.com/embed/ua-CiDNNj30",status :false,color :"yellow"},
         {name :"数据可视化",description :"使用Matplotlib进行绘图",ytv :"https://www.youtube.com/embed/-ETQ97mXXF0",status :false,color :"red"},
      ],
   },
   {
    name : "Développement d'Applications Mobiles",
    description : "Apprenez à créer des applications mobiles avec Flutter.",
    ytv : "https://www.youtube.com/embed/7C2b7bK4YfE",
    creatorName : "Émilie Dupont",
    creatorProfession : "Développeuse Mobile",
    creatorProfile : "https://example.com/profile_emilie.jpg",
    stars : 5,
    usersReviewed : 300,
    level : "All levels",
    status : false,
    language : "French",
    img : "https://images.unsplash.com/photo-1607252650355-f7fd0460ccdb?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTZ8fGFuZHJvaWQlMjBkZXZlbG9wbWVudHxlbnwwfHwwfHx8MA%3D%3D",
    content : [
       { name : "Introduction à Flutter", description : "Les bases de Flutter pour le développement mobile.", ytv : "https://www.youtube.com/embed/0sB1H4vPj1I", status : false, color : "orange" },
       { name : "Widgets Flutter", description : "Comprendre les widgets dans Flutter.", ytv : "https://www.youtube.com/embed/ZxKM3DCV2kE", status : false, color : "yellow" },
       { name : "Gestion d'État dans Flutter", description : "", ytv : "", status : false, color : "" },
    ],
 },                
];
  
export default coursesData;
  